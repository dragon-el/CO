...
script_description('Удобный помощник для Правительства.')
script_author('Dragon-El')
...
if not imguicheck or not sampevcheck or not encodingcheck or not lfscheck or not monetluacheck or not doesFileExist('moonloader/GOV Helper/Images/GOV_Images.png') then
	function main()
		if not isSampLoaded() or not isSampfuncsLoaded() then return end
		while not isSampAvailable() do wait(1000) end

		local GOVfont = renderCreateFont('trebucbd', 11, 9)
		...
		if not imguicheck then -- Нашел только релизную версию в архиве, так что пришлось залить файлы сюда, при обновлении буду обновлять и у себя
			...
			DownloadFiles({theme = 'mimgui',
				{url = 'https://raw.githubusercontent.com/dragon-el/GOV-Helper/main/mimgui/init.lua', file = 'moonloader/lib/mimgui/init.lua', name = 'init.lua'},
				{url = 'https://raw.githubusercontent.com/dragon-el/GOV-Helper/main/mimgui/imgui.lua', file = 'moonloader/lib/mimgui/imgui.lua', name = 'imgui.lua'},
				{url = 'https://raw.githubusercontent.com/dragon-el/GOV-Helper/main/mimgui/dx9.lua', file = 'moonloader/lib/mimgui/dx9.lua', name = 'dx9.lua'},
				{url = 'https://raw.githubusercontent.com/dragon-el/GOV-Helper/main/mimgui/cimguidx9.dll', file = 'moonloader/lib/mimgui/cimguidx9.dll', name = 'cimguidx9.dll'},
				{url = 'https://raw.githubusercontent.com/dragon-el/GOV-Helper/main/mimgui/cdefs.lua', file = 'moonloader/lib/mimgui/cdefs.lua', name = 'cdefs.lua'},
			})
		...
		if not encodingcheck then -- Обновлений быть не должно
			...
			DownloadFiles({ theme = 'encoding.lua',
				{url = 'https://raw.githubusercontent.com/dragon-el/GOV-Helper/main/encoding.lua', file = 'moonloader/lib/encoding.lua', name = 'encoding.lua'}
			})
		...
		if not lfscheck then -- Обновлений быть не должно
			...
			DownloadFiles({theme = 'lfs.dll',
				{url = 'https://raw.githubusercontent.com/dragon-el/GOV-Helper/main/lfs.dll', file = 'moonloader/lib/lfs.dll', name = 'lfs.dll'}
			})
			print('{00FF00}lfs успешно скачан')
		end

		if not doesFileExist('moonloader/GOV Helper/Images/GOV_Images.png') then
			print('{FFFF00}Скачивание: PNG Файлы')
			createDirectory('moonloader/GOV Helper')
			createDirectory('moonloader/GOV Helper/Images')
			DownloadFiles({theme = 'PNG Файлы',
				{url = 'https://raw.githubusercontent.com/dragon-el/GOV-Helper/main/Images/GOV_Images.png', file = 'moonloader/GOV Helper/Images/GOV_Images.png', name = 'GOV_Images.png'},
			})
...
local Licenses_Arr					= {u8'Паспорт',u8'Смена ДР',u8'[VC] Виза',u8'[VC] Перевод ТС'}
...
local pricelist = {
	passprice 						= new.char[7](tostring(configuration.main_settings.passprice)),
...
local departsettings = {
	myorgname						= new.char[50](u8(configuration.main_settings.pravotag)),
...
local tagbuttons = {
	...
	{name = '{delay_*}',text = 'Добавляет задержку между сообщениями',hint = 'Добрый день, я сотрудник Правительства г. Сан-Фиерро, чем могу Вам помочь?\n{delay_2000}\n/do На груди висит бейджик с надписью Лицензёр Правительства.\n\n[10:54:29] Добрый день, я сотрудник Правительства г. Сан-Фиерро, чем могу Вам помочь?\n[10:54:31] На груди висит бейджик с надписью Лицензёр Правительства.'},
}
local buttons = {
...
imgui.OnInitialize(function()
	...
	ash_image = imgui.CreateTextureFromFile(getGameDirectory()..'\\moonloader\\GOV Helper\\Images\\GOV_Images.png')
					...
					if imgui.Button(u8'Продать лицензию на '..u8(string.rlower(u8:decode(Licenses_Arr[Licenses_select[0]+1]))), imgui.ImVec2(285,30)) then
						...
						if lic ~= nil and to ~= nil then
							if (lic == 'оружие' and configuration.main_settings.checkmcongun) or (lic == 'охоту' and configuration.main_settings.checkmconhunt) then
								...
								sendchatarray(configuration.main_settings.playcd, {
									{'/me {gender:взял|взяла} со стола бланк и {gender:заполнил|заполнила} ручкой бланк на '..lic},
									{'/do Спустя некоторое время бланк на получение %s был заполнен.', lic},
							...
							if #str(uninvitebuf) > 0 then
								if uninvitebox[0] then
									if #str(blacklistbuf) > 0 then
										...
										GOVHelperMessage('Введите причину занесения в ЧС!')
									end
								else
									windows.imgui_fm[0] = false
									sendchatarray(configuration.main_settings.playcd, {
								...
								GOVHelperMessage('Введите причину увольнения.')
							end
						else
							GOVHelperMessage('Данная команда доступна с 9-го ранга.')
						end
				...
				if windowtype[0] == 6 then
					...
					if imgui.Button(u8'Выдать выговор '..sampGetPlayerNickname(fastmenuID)..'['..fastmenuID..']', imgui.ImVec2(285,30)) then
						if #str(fwarnbuff) > 0 then
							...
							GOVHelperMessage('Введите причину выдачи выговора!')
						end
					end
					imgui.SetCursorPos(imgui.ImVec2(7.5, 478))
					if imgui.Button(u8'Назад', imgui.ImVec2(142.5,30)) then
						...
						if configuration.main_settings.myrankint >= 9 then
							if #str(fmutebuff) > 0 then
								if tonumber(fmuteint[0]) and tonumber(fmuteint[0]) > 0 then
									...
									GOVHelperMessage('Введите корректное время мута!')
								end
							else
								GOVHelperMessage('Введите причину выдачи мута!')
							end
						else
							GOVHelperMessage('Данная команда доступна с 9-го ранга.')
						end
						...
						for k = 1, #serverquestions do
							...
							if imgui.Button(u8(serverquestions[k].name)..'##'..k, imgui.ImVec2(285, 30)) then
								if not inprocess then
									GOVHelperMessage('Подсказка: '..serverquestions[k].answer)
									sampSendChat(serverquestions[k].question)
									lastq[0] = clock()
								else
									GOVHelperMessage('Не торопитесь, Вы уже отыгрываете что-то! Прервать отыгровку: {MC}Alt{WC} + {MC}U{WC}')
								end
							end
						end
						...
						if #questions.questions ~= 0 then
							for k,v in pairs(questions.questions) do
								...
								if imgui.Button(u8(v.bname..'##'..k), imgui.ImVec2(questions.active.redact and 200 or 285,30)) then
									if not inprocess then
										GOVHelperMessage('Подсказка: '..v.bhint)
										sampSendChat(v.bq)
										lastq[0] = clock()
									else
										GOVHelperMessage('Не торопитесь, Вы уже отыгрываете что-то! Прервать отыгровку: {MC}Alt{WC} + {MC}U{WC}')
								...
								if questions.active.redact then
									...
									if imgui.Button(fa.ICON_FA_TRASH..'##'..k, imgui.ImVec2(30,30)) then
										...
										local file = io.open(getWorkingDirectory()..'\\GOV Helper\\Questions.json', 'w')
				...
				if windowtype[0] == 9 then
					if sobesetap[0] == 0 then
						...
						if imgui.Button(u8'Поприветствовать', imgui.ImVec2(285,30)) then
							sendchatarray(configuration.main_settings.playcd, {
								{'Здравствуйте, я %s %s, Вы пришли на собеседование?', configuration.RankNames[configuration.main_settings.myrankint], configuration.main_settings.replaceash and 'ГЦЛ' or 'Правительства'},
								{'/do На груди висит бейджик с надписью %s %s.', configuration.RankNames[configuration.main_settings.myrankint], #configuration.main_settings.myname < 1 and gsub(sampGetPlayerNickname(select(2,sampGetPlayerIdByCharHandle(playerPed))), '_', ' ') or u8:decode(configuration.main_settings.myname)},
							})
						end
						imgui.SetCursorPosX(7.5)
							...
							if not inprocess then
								local reasons = {
									pass = {
										...
										['в чс Правительства'] = {'К сожалению я не могу продолжить собеседование. Вы находитесь в ЧС АШ.'},
						...
						if clienttype[0] == 0 then
							...
							imgui.BeginGroup()
								...
								if configuration.main_settings.myrankint >= 2 then
									...
									imgui.LockedButton(fa.ICON_FA_FILE_ALT..u8' Выгнать из Правительства', imgui.ImVec2(285,30))
...
local imgui_settings = imgui.OnFrame(
		...
		imgui.Begin(u8'#MainSettingsWindow', windows.imgui_settings, imgui.WindowFlags.NoTitleBar + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.NoCollapse)
			...
			imgui.Image(ash_image,imgui.ImVec2(198,25),imgui.ImVec2(0.25,configuration.main_settings.style ~= 2 and 0.8 or 0.9),imgui.ImVec2(1,configuration.main_settings.style ~= 2 and 0.9 or 1))
				...
				if imgui.BeginPopupModal(u8'Все команды', _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.AlwaysAutoResize + imgui.WindowFlags.NoTitleBar) then
					...
					imgui.BeginGroup()
						imgui.TextColoredRGB('/gh - Главное меню скрипта')
						imgui.TextColoredRGB('/ghbind - Биндер скрипта')
						imgui.TextColoredRGB('/ghlect - Меню лекций скрипта')
						imgui.TextColoredRGB('/ghdep - Меню департамента скрипта')
					...
					imgui.BeginGroup()
						...
						imgui.TextColoredRGB('/expel [id] [причина] - Выгнать человека из Правительства (2+)')
					imgui.EndGroup()
					imgui.Spacing()
							...
							imgui.BeginGroup()
								...
								imgui.BeginGroup()
									...
									if imgui.Button(u8'Тест##ASTest',imgui.ImVec2(37,25)) then
										GOVHelperMessage('Это сообщение видите только Вы!')
									end
				...
				elseif mainwindow[0] == 2 then
					...
					if additionalwindow[0] == 1 then
						imgui.BeginChild('##rulesswindow',_,false, imgui.WindowFlags.NoScrollbar)
							...
							if imgui.IsMouseReleased(0) and imgui.IsItemHovered() then
								createDirectory(getWorkingDirectory()..'\\GOV Helper\\Rules')
								os.execute('explorer '..getWorkingDirectory()..'\\GOV Helper\\Rules')
							end
							imgui.SetCursorPos(imgui.ImVec2(15, 20))
									...
									if imgui.InvisibleButton('##zametkisave',imgui.ImVec2(85,15)) then
										if #str(zametkisettings.zametkaname) > 0 then
											if #str(zametkisettings.zametkatext) > 0 then
												...
												zametkaredact_number = nil
												local file = io.open(getWorkingDirectory()..'\\GOV Helper\\Zametki.json', 'w')
												file:write(encodeJson(zametki))
												file:close()
												updatechatcommands()
											else
												GOVHelperMessage('Текст заметки не введен.')
											end
										else
											GOVHelperMessage('Название заметки не введено.')
										end
									end
									imgui.SetCursorPos(imgui.ImVec2(220,190))
									imgui.PushFont(font[16])
				...
				elseif mainwindow[0] == 3 then
									...
									if imgui.Button(u8'Скачать '..fa.ICON_FA_ARROW_ALT_CIRCLE_DOWN) then
										local function DownloadFile(url, file)
											downloadUrlToFile(url,file,function(id,status)
												if status == dlstatus.STATUSEX_ENDDOWNLOAD then
													GOVHelperMessage('Обновление успешно загружено, скрипт перезагружается...')
												end
											end)
										end
							...
							elseif updateinfo.version and updateinfo.version == thisScript().version then
								...
								imgui.BeginGroup()
									...
									if imgui.Button(u8'Проверить наличие обновлений') then
										checkUpdates('https://raw.githubusercontent.com/dragon-el/GOV-Helper/main/Jsons/update.json', true)
									end
								imgui.EndGroup()
							else
								imgui.SetCursorPosY(30)
					...
					elseif infowindow[0] == 2 then
						...
						imgui.BeginGroup()
							...
							imgui.PushFont(font[15])
							imgui.TextColoredRGB('Автор - {MC}Dragon-El')
							imgui.PopFont()
							imgui.NewLine()
							...
							imgui.SameLine(190)
							imgui.Link('https://vk.com/maks.weeb', u8'vk.com/maks.weeb')
						    imgui.EndGroup()
					elseif infowindow[0] == 3 then
						imgui.SetCursorPos(imgui.ImVec2(15,15))
						imgui.BeginGroup()
							imgui.PushFont(font[16])
							imgui.TextColoredRGB('GOV Helper',1)
							imgui.PopFont()
	...
	* GOV Helper - удобный помощник, который облегчит Вам работу в Правительстве. Скрипт был разработан специально для проекта Arizona RP. Скрипт имеет открытый код для ознакомления, любое выставление скрипта без указания авторства запрещено! Обновления скрипта происходят безопасно для Вас, автообновления нет, установку должны подтверждать Вы.

	* Меню быстрого доступа - Прицелившись на игрока с помощью ПКМ и нажав кнопку E (по умолчанию), откроется меню быстрого доступа. В данном меню есть все нужные функции, а именно: приветствие, озвучивание прайс листа, продажа услуг мерии, возможность выгнать человека из мерии, приглашение в организацию, увольнение из организации, изменение должности, занесение в ЧС, удаление из ЧС, выдача выговоров, удаление выговоров, выдача организационного мута, удаление организационного мута, автоматизированное проведение собеседования со всеми нужными отыгровками.
	...
	* Команды хелпера - /gh - настройки хелпера, /ghbind - биндер хелпера, /ghlect - меню лекций, /ghdep - меню департамента

	* Настройки - Введя команду /gh откроются настройки в которых можно изменять никнейм в приветствии, акцент, создание маркера при выделении, пол, цены на лицензии, горячую клавишу быстрого меню и многое другое.

	* Меню лекций - Введя команду /ghlect откроется меню лекций, в котором вы сможете озвучить/добавить/удалить лекции.

	* Биндер - Введя команду /ghbind откроется биндер, в котором вы можете создать абсолютно любой бинд на команду, или же кнопку(и).]])
						imgui.Spacing()
						imgui.EndGroup()
					end
					imgui.EndChild()
		...
		if imgui.BeginPopup(u8'Тэги', nil, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.AlwaysAutoResize + imgui.WindowFlags.NoTitleBar) then
			for k,v in pairs(tagbuttons) do
				if imgui.Button(u8(tagbuttons[k].name),imgui.ImVec2(150,25)) then
					...
					GOVHelperMessage('Тэг был скопирован.')
				end
				imgui.SameLine()
				if imgui.IsItemHovered() then
					imgui.BeginTooltip()
				...
				if imgui.Button(u8'Сохранить',imgui.ImVec2(100,30)) then
					...
					if not inprocess then
						...
						if kei ~= nil then
							...
							if inicfg.save(configuration, 'GOV Helper') then
								GOVHelperMessage('Бинд успешно сохранён!')
							end
						else
							...
							if inicfg.save(configuration, 'GOV Helper') then
								GOVHelperMessage('Бинд успешно создан!')
							end
						end
						...
						GOVHelperMessage('Вы не можете взаимодействовать с биндером во время любой отыгровки!')
					end	
				end
			else
					...
					for key, value in pairs(configuration.BindsName) do
						...
						if u8:decode(str(bindersettings.bindername)) == configuration.BindsName[key] then
							...
							if inicfg.save(configuration,'GOV Helper') then
								...
								bindersettings.bindertype[0] = 0
								choosedslot = nil
								GOVHelperMessage('Бинд успешно удалён!')
							end
						end
					end
					updatechatcommands()
				else
					GOVHelperMessage('Вы не можете удалять бинд во время любой отыгровки!')
				end
			end
		else
	...
	function(player)
		...
		if #lections.data == 0 then
			...
			if imgui.Button(u8'Восстановить изначальные лекции', imgui.ImVec2(250, 25)) then
				...
				local file = io.open(getWorkingDirectory()..'\\GOV Helper\\Lections.json', 'w')
	...
	function(player)
		...
		if imgui.Button(fa.ICON_FA_MINUS_SQUARE,imgui.ImVec2(23,23)) then
			if #dephistory ~= 0 then
				dephistory = {}
				GOVHelperMessage('История сообщений успешно очищена.')
			end
		end
		...
		imgui.BeginChild('##depbuttons',imgui.ImVec2(180,300),true, imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.NoScrollWithMouse)
			...
			if imgui.InputTextWithHint('##myorgnamedep',u8('Правительство'),departsettings.myorgname, sizeof(departsettings.myorgname)) then
				configuration.main_settings.pravotag = u8:decode(str(departsettings.myorgname))
			end
			...
			if imgui.Button(u8'Перейти на частоту',imgui.ImVec2(150,25)) then
				if #str(departsettings.frequency) > 0 and #str(departsettings.myorgname) > 0 then
					...
					GOVHelperMessage('У Вас что-то не указано.')
				end
			end
			...
			if notify.msg[k] and notify.msg[k].active then
				...
				if notify.pos.y - i * 21 > 0 then
					...
					imgui.Begin(u8('Notify ##'..k), _, imgui.WindowFlags.NoTitleBar + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoScrollbar)
						...
						imgui.TextColoredRGB('{MC}GOV Helper')
						imgui.PopFont()
...
function sampev.onShowDialog(dialogId, style, title, button1, button2, text)
	if dialogId == 6 and givelic then
		local d = {
			['паспорт'] = 0,
			['изменить дату рождения в паспорте'] = 1,
			['выдать визу'] = 3,
			['перевести транспорт'] = 4,
		}
		sampSendDialogResponse(6, 1, d[lictype], nil)
		lua_thread.create(function()
			wait(1000)
			if givelic then
				sampSendChat(format('/givepass %s',sellto))
			end
		end)
		return false

	elseif dialogId == 235 and getmyrank then
		if find(text, 'Правительство') then
			for DialogLine in gmatch(text, '[^\r\n]+') do
				...
				if tonumber(getStatsRank) then
					...
					if rangint ~= configuration.main_settings.myrankint then
						GOVHelperMessage(format('Ваш ранг был обновлён на %s (%s)',rang,rangint))
					end
					if configuration.RankNames[rangint] ~= rang then
						GOVHelperMessage(format('Название {MC}%s{WC} ранга изменено с {MC}%s{WC} на {MC}%s{WC}', rangint, configuration.RankNames[rangint], rang))
					end
			...
			print('{FF0000}Игрок не работает в Правительстве. Скрипт был выгружен.')
			GOVHelperMessage('Вы не работаете в Правительстве, скрипт выгружен! Если это ошибка, то обратитесь к {MC}vk.com/maks.weeb{WC}.')
			NoErrors = true
			thisScript():unload()
		end
			...
			if skiporcancel then
				if find(text, 'Имя: '..sampGetPlayerNickname(tempid)) then
					if inprocess then
						inprocess:terminate()
						inprocess = nil
						GOVHelperMessage('Прошлая отыгровка была прервана, из-за показа мед. карты.')
					end
					if find(text, 'Полностью здоровый') then
						...
						}, function() skiporcancel = false GOVHelperMessage('Человек не полностью здоровый, требуется поменять мед.карту!') end)
					end
		...
		elseif find(text, 'Серия') then
			if configuration.sobes_settings.pass and sobes_results and not sobes_results.pass then
				...
				if find(text, 'Состоит в ЧС{FF6200} Инструкторы') then
					sobes_results.pass = ('в чс Правительства')
					return {dialogId, style, title, button1, button2, text}
				end
		...
		for line in gmatch(text, '[^\r\n]+') do
			...
			if name and rank then
				...
				if configuration.RankNames[rank] ~= nil and configuration.RankNames[rank] ~= name then
					GOVHelperMessage(format('Название {MC}%s{WC} ранга изменено с {MC}%s{WC} на {MC}%s{WC}', rank, configuration.RankNames[rank], name))
...
function sampev.onServerMessage(color, message)
	if configuration.main_settings.replacechat then
		if find(message, 'Используйте: /jobprogress %[ ID игрока %]') then
			GOVHelperMessage('Вы просмотрели свою рабочую успеваемость.')
			return false
		end
...
function sampev.onSendChat(message)
	...
	if find(message, '{close_id}') then
		...
		GOVHelperMessage('В зоне стрима не найдено ни одного игрока.')
		return false
	end
	if find(message, '@{%d+}') then
		...
		GOVHelperMessage('Такого игрока нет на сервере.')
		return false
	end
				...
				if find(cmd, '/'..configuration.main_settings.usefastmenucmd..' %d+') then
					...
					if sampIsPlayerConnected(param) then
						if doesCharExist(select(2,sampGetCharHandleBySampPlayerId(param))) then
							fastmenuID = param
							GOVHelperMessage(format('Вы использовали меню быстрого доступа на: %s [%s]',gsub(sampGetPlayerNickname(fastmenuID), '_', ' '),fastmenuID))
							GOVHelperMessage('Зажмите {MC}ALT{WC} для того, чтобы скрыть курсор. {MC}ESC{WC} для того, чтобы закрыть меню.')
							windows.imgui_fm[0] = true
						else
							GOVHelperMessage('Игрок не находится рядом с вами')
						end
					else
						GOVHelperMessage('Игрок не в сети')
					end
				else
					GOVHelperMessage('/'..configuration.main_settings.usefastmenucmd..' [id]')
				end
			end
			return false
		end
...
function createJsons()
	createDirectory(getWorkingDirectory()..'\\GOV Helper')
	createDirectory(getWorkingDirectory()..'\\GOV Helper\\Rules')
	if not doesFileExist(getWorkingDirectory()..'\\GOV Helper\\Lections.json') then
		lections = default_lect
		local file = io.open(getWorkingDirectory()..'\\GOV Helper\\Lections.json', 'w')
		file:write(encodeJson(lections))
		file:close()
	else
		local file = io.open(getWorkingDirectory()..'\\GOV Helper\\Lections.json', 'r')
		lections = decodeJson(file:read('*a'))
		file:close()
	end
	if not doesFileExist(getWorkingDirectory()..'\\GOV Helper\\Questions.json') then
		questions = {
			active = { redact = false },
			questions = {}
		}
		...
		local file = io.open(getWorkingDirectory()..'\\GOV Helper\\Questions.json', 'r')
		questions = decodeJson(file:read('*a'))
		questions.active.redact = false
		file:close()
	end
	if not doesFileExist(getWorkingDirectory()..'\\GOV Helper\\Zametki.json') then
		zametki = {}
		local file = io.open(getWorkingDirectory()..'\\GOV Helper\\Zametki.json', 'w')
		file:write(encodeJson(zametki))
		file:close()
	else
		local file = io.open(getWorkingDirectory()..'\\GOV Helper\\Zametki.json', 'r')
...
function checkRules()
	ruless = {}
	for line in lfs.dir(getWorkingDirectory()..'\\GOV Helper\\Rules') do
		if line == nil then
		elseif line:match('.+%.txt') then
			local temp = io.open(getWorkingDirectory()..'\\GOV Helper\\Rules\\'..line:match('.+%.txt'), 'r+')
			local temptable = {}
			...
			if doesFileExist(json) then
				...
				if f then
					if info[checkServer(select(1, sampGetCurrentServerAddress()))] then
						...
						GOVHelperMessage('Ошибка в импортировании правил сервера! Неизвестный сервер.')
					end
				end
			end
		end
	...
	local json_url = 'https://raw.githubusercontent.com/dragon-el/GOV-Helper/main/Jsons/questions.json'
			...
			if doesFileExist(json) then
				...
				if f then
					if info[checkServer(select(1, sampGetCurrentServerAddress()))] then
						...
						GOVHelperMessage('Ошибка в импортировании устава сервера! Неизвестный сервер.')
					end
				end
			end
		end
...
function main()
	...
	sampRegisterChatCommand('ghbind', function()
		choosedslot = nil
		windows.imgui_binder[0] = not windows.imgui_binder[0]
	end)
	sampRegisterChatCommand('ghstats', function()
		GOVHelperMessage('Это окно теперь включается в {MC}/gh{WC} - {MC}Настройки{WC}.')
	end)
	...
	sampRegisterChatCommand('ghupd', function()
	...
	sampRegisterChatCommand('uninvite', function(param)
		...
		if configuration.main_settings.myrankint < 9 then
			return GOVHelperMessage('Данная команда доступна с 9-го ранга.')
		end
		...
		if uvalid == nil or reason == nil then
			return GOVHelperMessage('/uninvite [id] [причина], [причина чс] (не обязательно)')
		end
		if tonumber(uvalid) == select(2,sampGetPlayerIdByCharHandle(playerPed)) then
			return GOVHelperMessage('Вы не можете увольнять из организации самого себя.')
		end
		if withbl then
	...
	sampRegisterChatCommand('invite', function(param)
		...
		if id == nil then
			return GOVHelperMessage('/invite [id]')
		end
		if tonumber(id) == select(2,sampGetPlayerIdByCharHandle(playerPed)) then
			return GOVHelperMessage('Вы не можете приглашать в организацию самого себя.')
		end
		return sendchatarray(configuration.main_settings.playcd, {
			{'/do Ключи от шкафчика в кармане.'},
			{'/me всунув руку в карман брюк, {gender:достал|достала} оттуда ключ от шкафчика'},
	...
	sampRegisterChatCommand('giverank', function(param)
		...
		if id == nil or rank == nil then
			return GOVHelperMessage('/giverank [id] [ранг]')
		end
		if tonumber(id) == select(2,sampGetPlayerIdByCharHandle(playerPed)) then
			return GOVHelperMessage('Вы не можете менять ранг самому себе.')
		end
		return sendchatarray(configuration.main_settings.playcd, {
			{'/me {gender:включил|включила} планшет'},
			{'/me {gender:перешёл|перешла} в раздел \'Управление сотрудниками\''},
	...
	sampRegisterChatCommand('blacklist', function(param)
		...
		if id == nil or reason == nil then
			return GOVHelperMessage('/blacklist [id] [причина]')
		end
		if tonumber(id) == select(2,sampGetPlayerIdByCharHandle(playerPed)) then
			return GOVHelperMessage('Вы не можете внести в ЧС самого себя.')
		end
		return sendchatarray(configuration.main_settings.playcd, {
			{'/me {gender:достал|достала} планшет из кармана'},
			{'/me {gender:перешёл|перешла} в раздел \'Чёрный список\''},
	...
	sampRegisterChatCommand('unblacklist', function(param)
		...
		local id = param:match('(%d+)')
		if id == nil then
			return GOVHelperMessage('/unblacklist [id]')
		end
		return sendchatarray(configuration.main_settings.playcd, {
			{'/me {gender:достал|достала} планшет из кармана'},
			{'/me {gender:перешёл|перешла} в раздел \'Чёрный список\''},
	...
	sampRegisterChatCommand('fwarn', function(param)
		...
		local id,reason = param:match('(%d+) (.+)')
		if id == nil or reason == nil then
			return GOVHelperMessage('/fwarn [id] [причина]')
		end
		return sendchatarray(configuration.main_settings.playcd, {
			{'/me {gender:достал|достала} планшет из кармана'},
			{'/me {gender:перешёл|перешла} в раздел \'Управление сотрудниками\''},
	...
	sampRegisterChatCommand('unfwarn', function(param)
		...
		local id = param:match('(%d+)')
		if id == nil then
			return GOVHelperMessage('/unfwarn [id]')
		end
		return sendchatarray(configuration.main_settings.playcd, {
			{'/me {gender:достал|достала} планшет из кармана'},
			{'/me {gender:перешёл|перешла} в раздел \'Управление сотрудниками\''},
	...
	sampRegisterChatCommand('fmute', function(param)
		...
		if id == nil or reason == nil or mutetime == nil then
			return GOVHelperMessage('/fmute [id] [время] [причина]')
		end
		return sendchatarray(configuration.main_settings.playcd, {
			{'/me {gender:достал|достала} планшет из кармана'},
			{'/me {gender:включил|включила} планшет'},
	...
	sampRegisterChatCommand('funmute', function(param)
		...
		local id = param:match('(%d+)')
		if id == nil then
			return GOVHelperMessage('/funmute [id]')
		end
		return sendchatarray(configuration.main_settings.playcd, {
			{'/me {gender:достал|достала} планшет из кармана'},
			{'/me {gender:включил|включила} планшет'},
	...
	sampRegisterChatCommand('expel', function(param)
		...
		if configuration.main_settings.myrankint < 2 then
			return GOVHelperMessage('Данная команда доступна с 2-го ранга.')
		end
		local id,reason = param:match('(%d+) (.+)')
		if id == nil or reason == nil then
			return GOVHelperMessage('/expel [id] [причина]')
		end
		if sampIsPlayerPaused(id) then
			return GOVHelperMessage('Игрок находится в АФК!')
		end
		return sendchatarray(configuration.main_settings.playcd, {
			{'/me схватил человека за руку, и повел к выходу'},
			{'/me открыв дверь рукой, вывел человека на улицу'},
	...
	while true do
				...
				if sampGetPlayerIdByCharHandle(select(2,getCharPlayerIsTargeting())) then
					if configuration.main_settings.myrankint > 2 then
						...
						if #reason > 0 then
							...
							GOVHelperMessage('/expel [id] [причина]')
						end
					else
						GOVHelperMessage('Данное действие доступно с 2-го ранга.')
					end
				end
			end
		end
		...
		if inprocess and isKeyDown(0x12) and isKeyJustPressed(0x55) then
			inprocess:terminate()
			inprocess = nil
			GOVHelperMessage('Отыгровка успешно прервана!')
		end
		...
		if skiporcancel and isKeyDown(0x12) and isKeyJustPressed(0x4F) then
			...
			GOVHelperMessage('Ожидание мед. карты остановлено!')
		end
		...
		if configuration.main_settings.autoupdate and clock() - autoupd[0] > 600 then
			checkUpdates('https://raw.githubusercontent.com/dragon-el/GOV-Helper/main/Jsons/update.json')
			autoupd[0] = clock()
		end
...
changelog = {
	versions = {
...

		{
			...
			date = '01.05.2021',
			text = {
				'Меню статистики /ghstats сделано более удобным',
				'Двойной клик сохранит местоположение статистики /ghstats',
				'Изменены кнопки в главном меню /gh',
				'Теперь можно тестировать цвета в /gh',
				'Теперь при активации бинда последнее сообщение будет без задержки',
				'Добавлен список изменений',
				'Исправлена проверка обновлений через /gh',
				'Кардинальный редизайн главного меню /gh',
 ...
 - Исправлен баг с ударом при принятии человека в организацию]]
...

		{
			...
			text = {
				...
				'Добавлена функция озвучивания лекций /ghlect',
 			...
 			patches = {
				active = false,
				text = [[
 - Исправлен краш при открытии /ghstats
 ...
 - Исправлен баг с пропадающим курсором после использования быстрого меню]]
...

		{
			...
			date = '16.05.2021',
			text = {
				'Добавлена функция общения в рацию департамента /ghdep',
 ...
 - Испралена неработоспособность скрипта из-за перехода сервера на R3]]
		...
		
		{
			...
			date = '11.12.2021',
			text = {
				'Изменение интерфейса в списке изменений и /gh',
 ...
 - Исправлен баг с неработающей командой /ghupd
...
default_lect = {
	...
	data = {
		...
		{
			name = 'Субординация в Правительстве',
			text = {
				'Дорогие сотрудники! Минуточку внимания.',
				'Прошу вас соблюдать Субординацию в Правительстве...',
		...
		},
		{
			...
			text = {
				'Cейчас я проведу лекцию на тему \'Основные правила Правительства\'.',
				'Сотрудникам Правительства запрещено прогуливать рабочий день.',
				'Сотрудникам Правительства запрещено в рабочее время посещать мероприятия.',
				'Сотрудникам Правительства запрещено в рабочее время посещать казино.',
				'Сотрудникам Правительства запрещено в рабочее время посещать любые подработки.',
				'Сотрудникам Правительства запрещено носить при себе огнестрельное оружие.',
				'Сотрудникам Правительства запрещено курить в здании Правительства.',
				'Сотрудникам Правительства запрещено употреблять алкогольные напитки в рабочее время.',
		...
		},
		{
			...
			text = {
				...
				'Сотрудники в рабочее время обязаны находиться в офисе Правительства в форме.',
				...
				'В не рабочее время или в обед сотрудник может покинуть офис Правительства.',
